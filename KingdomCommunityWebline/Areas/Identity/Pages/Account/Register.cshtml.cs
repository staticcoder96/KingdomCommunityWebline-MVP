...
namespace KingdomCommunityWebline.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                //You want to block unapproved users BEFORE account creation starts.
                var approvedMember = _context.ApprovedMembers.FirstOrDefault(m => m.EmailAddress == Input.Email);

                if (approvedMember == null)
                {
                    ModelState.AddModelError(string.Empty, "You are not an aproved church member.");
                    return Page();
                }

                //prevent double registration
                if (approvedMember.IsRegistered)
                {
                    ModelState.AddModelError(string.Empty, "This member already exists.");
                    return Page();                    
                }
